...
export default class FlowToolOriginal02 {
    public events: any = {};
    public container: HTMLElement | any;
    public precanvas: HTMLElement | any;
    public nodeId = 1;
    public ele_selected: any = null;
    public node_selected: any = null;
    public drag: any = false;
    public reroute: any = false;
    public reroute_fix_curvature: any = false;
    public curvature: any = 0.5;
    public reroute_curvature_start_end: any = 0.5;
    public reroute_curvature: any = 0.5;
    public reroute_width: any = 6;
    public drag_point: any = false;
    public editor_selected: any = false;
    public connection: any = false;
    public connection_ele: any = null;
    public connection_selected: any = null;
    public canvas_x: any = 0;
    public canvas_y: any = 0;
    public pos_x: any = 0;
    public pos_x_start: any = 0;
    public pos_y: any = 0;
    public pos_y_start: any = 0;
    public mouse_x: any = 0;
    public mouse_y: any = 0;
    public line_path: any = 5;
    public first_click: any = null;
    public force_first_input: any = false;
    public draggable_inputs: any = true;
    public useuuid: any = false;
    public parent: any;

    public noderegister: any = {};
    public render: any;

    public drawflowTest: Array<DrawFlowModel> = [];

    // Configurable options
    public module: string = 'Home';
    public editor_mode: string = 'edit';
    public zoom: number = 1;
    public zoom_max: number = 1.6;
    public zoom_min: number = 0.5;
    public zoom_value: number = 0.1;
    public zoom_last_value: number = 1;

    // Mobile
    public evCache = new Array();
    public prevDiff = -1;
    ...
    start () {
      ...
      /* Zoom Mouse */
    //   this.container.addEventListener('wheel', this.zoom_enter.bind(this)); // Commented this out for now - shannon
      /* Update data Nodes */
    ...
    protected activeModule(module?: string): DrawFlowModel | any {
        ...
        const t = this.drawflowTest.find((e: DrawFlowModel) => {
            if (module) {
                return e.module === module;
            }

            return e.module === this.module;
        })
    ...
    load() {
        ...
        for (var key in this.activeModule(this.module).data) {
            this.addNodeImport(this.activeModule(this.module).data[key], this.precanvas);
          }
      ...
      if(this.reroute) {
        for (var key in this.activeModule(this.module).data) {
          this.addRerouteImport(this.activeModule(this.module).data[key]);
        }
      }
      ...
      const editor: any = this.activeModule(this.module);
      let number = 1;
    ...
    position(e: any) {
      ...
      if(this.drag) {
        ...
        this.activeModule(this.module).data[this.ele_selected.id.slice(5)].pos_x = (this.ele_selected.offsetLeft - x);
        this.activeModule(this.module).data[this.ele_selected.id.slice(5)].pos_y = (this.ele_selected.offsetTop - y);
      ...
      if(this.drag_point) {
        ...
        const searchConnection = this.activeModule(this.module).data[nodeId].outputs[output_class].connections.findIndex(function(item: any,i: any) {
          return item.node ===  nodeUpdateIn && item.output === input_class;
        });
  
        this.activeModule(this.module).data[nodeId].outputs[output_class].connections[searchConnection].points[numberPointPosition] = { pos_x: pos_x, pos_y: pos_y };
        ...
        if(ele_last.classList[0] === 'input' || (this.force_first_input && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node'))) {
         ...
         var output_class = this.ele_selected.classList[1];
          ...
          if(output_id !== input_id && input_class !== false) {
            ...
            this.activeModule(this.module).data[id_output].outputs[output_class].connections.push( {"node": id_input, "output": input_class});
            this.activeModule(this.module).data[id_input].inputs[input_class].connections.push( {"node": id_output, "input": output_class});
    ...
    createReroutePoint(ele: any) {
        ...
        if(this.activeModule(this.module).data[nodeId].outputs[output_class].connections[searchConnection].points === undefined)  {
            this.activeModule(this.module).data[nodeId].outputs[output_class].connections[searchConnection].points = [];
        }
  
        if(this.reroute_fix_curvature) {
          //console.log(position_add_array_point)
          if(position_add_array_point > 0) {
            this.activeModule(this.module).data[nodeId].outputs[output_class].connections[searchConnection].points.splice(position_add_array_point, 0, { pos_x: pos_x, pos_y: pos_y });
          } else {
            this.activeModule(this.module).data[nodeId].outputs[output_class].connections[searchConnection].points.push({ pos_x: pos_x, pos_y: pos_y });
          }
    ...
    removeReroutePoint(ele: any) {
      ...
      this.activeModule(this.module).data[nodeId].outputs[output_class].connections[searchConnection].points.splice(numberPointPosition,1);
    ...
    getNodeFromId(id: any) {
      ...
      return JSON.parse(JSON.stringify(this.activeModule(this.module).data[id]));
    }
    ...
    addNode (name: any, num_in: any, num_out: any, ele_pos_x: any, ele_pos_y: any, classoverride: any, data: any, html: any, typenode = false) {
      ...
      if(typenode === false) {
        ...
        if(parseInt(this.render.version) === 3 ) {
          ...
          let wrapper = this.render.createApp({
            parent: this.parent,
            render: (h: any) => this.render.h(this.noderegister[html].html, this.noderegister[html].props, this.noderegister[html].options)
          }).mount(content)
        } else {
          // Vue 2
      ...
      this.activeModule(this.module).data[newNodeId] = json;
    ...
    addNodeImport (dataNode: any, precanvas: any) {
      ...
      if(dataNode.typenode === false) {
        ...
        if(parseInt(this.render.version) === 3 ) {
          ...
          let wrapper = this.render.createApp({
            parent: this.parent,
            render: (h: any) => this.render.h(this.noderegister[dataNode.html].html, this.noderegister[dataNode.html].props, this.noderegister[dataNode.html].options)
          }).mount(content)
        } else {
          //Vue 2
          let wrapper = new this.render({
    ...
    updateNodeValue(event: any) {
      ...
      for (var i = 0; i < attr.length; i++) {
              if (attr[i].nodeName.startsWith('df-')) {
                  ...
                  var target = this.activeModule(this.module).data[event.target.closest(".drawflow_content_node").parentElement.id.slice(5)].data;
    ...
    updateNodeDataFromId(id: any, data: any) {
      var moduleName: any = this.getModuleFromNodeId(id)
      this.activeModule(this.module).data[id].data = data;
      if(this.module === moduleName) {
        const content: any = this.container.querySelector('#node-'+id);
    ...
    addNodeInput(id: any) {
      ...
      if(this.module === moduleName) {
        ...
        const parent: any = this.container.querySelector('#node-'+id+' .inputs');
        parent.appendChild(input);
        this.updateConnectionNodes('node-'+id);
  
      }
      this.activeModule(this.module).data[id].inputs["input_"+(numInputs+1)] = { "connections": []};
    }
    ...
    addNodeOutput(id: any) {
      ...
      if(this.module === moduleName) {
        ...
        const parent: any = this.container.querySelector('#node-'+id+' .outputs');
        parent.appendChild(output);
        this.updateConnectionNodes('node-'+id);
  
      }
      this.activeModule(this.module).data[id].outputs["output_"+(numOutputs+1)] = { "connections": []};
    }
    ...
    removeNodeInput(id: any, input_class: any) {
      ...
      delete this.activeModule(this.module).data[id].inputs[input_class];
  
      // Update connection
      const connections: Array<any> = [];
      const connectionsInputs = this.activeModule(this.module).data[id].inputs
      Object.keys(connectionsInputs).map(function(key, index) {
        connections.push(connectionsInputs[key]);
      });
      this.activeModule(this.module).data[id].inputs = {};
      ...
      connections.forEach((item, i) => {
        ...
        this.activeModule(this.module).data[id].inputs['input_'+ (i+1)] = item;
      });
      ...
      nodeUpdates.forEach((itemx: any, i: any) => {
        this.activeModule(this.module).data[itemx.node].outputs[itemx.input].connections.forEach((itemz: any, g: any) => {
            if(itemz.node == id) {
              const output_id = itemz.output.slice(6);
              if(parseInt(input_class_id) < parseInt(output_id)) {
                if(this.module === moduleName) {
                  const ele: any = this.container.querySelector(".connection.node_in_node-"+id+".node_out_node-"+itemx.node+"."+itemx.input+".input_"+output_id);
                  ele.classList.remove('input_'+output_id);
                  ele.classList.add('input_'+(output_id-1));
                }
                if(itemz.points) {
                    this.activeModule(this.module).data[itemx.node].outputs[itemx.input].connections[g] = { node: itemz.node, output: 'input_'+(output_id-1), points: itemz.points }
                } else {
                    this.activeModule(this.module).data[itemx.node].outputs[itemx.input].connections[g] = { node: itemz.node, output: 'input_'+(output_id-1)}
                }
              }
            }
        });
    ...
    removeNodeOutput(id: any, output_class: string) {
      ...
      delete this.activeModule(this.module).data[id].outputs[output_class];
  
      // Update connection
      const connections: Array<any> = [];
      const connectionsOuputs = this.activeModule(this.module).data[id].outputs
      Object.keys(connectionsOuputs).map(function(key, index) {
        connections.push(connectionsOuputs[key]);
      });
      this.activeModule(this.module).data[id].outputs = {};
      ...
      connections.forEach((item, i) => {
        ...
        this.activeModule(this.module).data[id].outputs['output_'+ (i+1)] = item;
      });
      nodeUpdates =  new Set(nodeUpdates.map((e: any) => JSON.stringify(e)));
      nodeUpdates = Array.from(nodeUpdates).map((e: any) => JSON.parse(e));
      ...
      nodeUpdates.forEach((itemx: any, i: any) => {
        this.activeModule(this.module).data[itemx.node].inputs[itemx.output].connections.forEach((itemz: any, g: any) => {
            if(itemz.node == id) {
              const input_id = itemz.input.slice(7);
              if(parseInt(output_class_id) < parseInt(input_id)) {
                if(this.module === moduleName) {
  
                  const ele: any = this.container.querySelector(".connection.node_in_node-"+itemx.node+".node_out_node-"+id+".output_"+input_id+"."+itemx.output);
                ...
                if(itemz.points) {
                    this.activeModule(this.module).data[itemx.node].inputs[itemx.output].connections[g] = { node: itemz.node, input: 'output_'+(input_id-1), points: itemz.points }
                } else {
                    this.activeModule(this.module).data[itemx.node].inputs[itemx.output].connections[g] = { node: itemz.node, input: 'output_'+(input_id-1)}
                }
              }
            }
        });
    ...
    removeNodeId(id: any) {
      ...
      delete this.activeModule(this.module).data[id.slice(5)];
    ...
    removeConnection() {
      if(this.connection_selected != null) {
        ...
        var index_out = this.activeModule(this.module).data[listclass[2].slice(14)].outputs[listclass[3]].connections.findIndex(function(item: any,i: any) {
          return item.node === listclass[1].slice(13) && item.output === listclass[4]
        });
        this.activeModule(this.module).data[listclass[2].slice(14)].outputs[listclass[3]].connections.splice(index_out,1);
  
        var index_in = this.activeModule(this.module).data[listclass[1].slice(13)].inputs[listclass[4]].connections.findIndex(function(item: any,i: any) {
          return item.node === listclass[2].slice(14) && item.input === listclass[3]
        });
        this.activeModule(this.module).data[listclass[1].slice(13)].inputs[listclass[4]].connections.splice(index_in,1);
    ...
    //   this.drawflow.drawflow[name] =  { "data": {} };
      ...
      this.drawflowTest.push(newModule)
    ...
    changeModule(name: any) {
      ...
      this.import(this.activeModule(this.module), false);
    }
    ...
    removeModule(name: any) {
      ...
      const index: number = this.drawflowTest.findIndex((e:DrawFlowModel) => {
        return e.module === 'name';
      });

      if (index !== -1) {
          this.drawflowTest.splice(index, 1);
      }
    ...
    clearModuleSelected() {
      ...
      this.drawflowTest.find((e: DrawFlowModel) => {
        if (e.module === this.module) {
            e.data = {};
        }
      })

      // this.activeModule(this.module) =  { "data": {} };
    }
    ...
    import (data: any, notifi = true) {
      this.clear();
       // this.drawflow = JSON.parse(JSON.stringify(data));
        ...
        val.data = JSON.parse(JSON.stringify(data)).drawflow.Home.data;
        val.module = 'Home';

      this.drawflowTest.push(val);
