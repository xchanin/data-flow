...
export default class FlowToolOriginal02 extends DataFlowBaseClass {
    ...
    load() {
        ...
        const editor = this.activeModule(Variables.module);
        let number = 1;
        ...
        if (typenode === false) {
            ...
            if (parseInt(Variables.render.version) === 3) {
                ...
                let wrapper = Variables.render.createApp({
                    ...
                    render: (h) => Variables.render.h(Variables.noderegister[html].html, Variables.noderegister[html].props, Variables.noderegister[html].options)
                }).mount(content);
            }
            else {
                // Vue 2
                let wrapper = new Variables.render(Object.assign({ parent: Variables.parent, render: (h) => h(Variables.noderegister[html].html, { props: Variables.noderegister[html].props }) }, Variables.noderegister[html].options)).$mount();
                //
    ...
    addNodeImport(dataNode, precanvas) {
        ...
        if (dataNode.typenode === false) {
            ...
            if (parseInt(Variables.render.version) === 3) {
                ...
                let wrapper = Variables.render.createApp({
                    parent: Variables.parent,
                    render: (h) => Variables.render.h(Variables.noderegister[dataNode.html].html, Variables.noderegister[dataNode.html].props, Variables.noderegister[dataNode.html].options)
                }).mount(content);
            }
            else {
                //Vue 2
                let wrapper = new Variables.render(Object.assign({ parent: Variables.parent, render: (h) => h(Variables.noderegister[dataNode.html].html, { props: Variables.noderegister[dataNode.html].props }) }, Variables.noderegister[dataNode.html].options)).$mount();
                content.appendChild(wrapper.$el);
            }
        }
    ...
    updateNodeDataFromId(id, data) {
        ...
        if (Variables.module === moduleName) {
            const content = Variables.container.querySelector('#node-' + id);
            Object.entries(data).forEach(function (key, value) {
                if (typeof key[1] === "object") {
                    insertObjectkeys(null, key[0], key[0]);
                }
    ...
    addNodeInput(id) {
        ...
        if (Variables.module === moduleName) {
            ...
            const parent = Variables.container.querySelector('#node-' + id + ' .inputs');
    ...
    addNodeOutput(id) {
        ...
        if (Variables.module === moduleName) {
            ...
            const parent = Variables.container.querySelector('#node-' + id + ' .outputs');
    ...
    removeNodeInput(id, input_class) {
        ...
        nodeUpdates.forEach((itemx, i) => {
            this.activeModule(Variables.module).Data[itemx.node].outputs[itemx.input].connections.forEach((itemz, g) => {
                if (itemz.node == id) {
                    const output_id = itemz.output.slice(6);
                    if (parseInt(input_class_id) < parseInt(output_id)) {
                        if (Variables.module === moduleName) {
                            const ele = Variables.container.querySelector(".connection.node_in_node-" + id + ".node_out_node-" + itemx.node + "." + itemx.input + ".input_" + output_id);
    ...
    removeNodeOutput(id, output_class) {
        ...
        nodeUpdates.forEach((itemx, i) => {
            this.activeModule(Variables.module).Data[itemx.node].inputs[itemx.output].connections.forEach((itemz, g) => {
                if (itemz.node == id) {
                    const input_id = itemz.input.slice(7);
                    if (parseInt(output_class_id) < parseInt(input_id)) {
                        if (Variables.module === moduleName) {
                            const ele = Variables.container.querySelector(".connection.node_in_node-" + itemx.node + ".node_out_node-" + id + ".output_" + input_id + "." + itemx.output);
    ...
    addModule(name) {
        ...
        const newModule = new DrawFlowModel({
            Data: {},
            Module: name
        });
    ...
    removeModule(name) {
        ...
        const index = Variables.drawflowTest.findIndex((e) => {
    ...
    removeListener(event, callback) {
        ...
        Variables.events[event].listeners = Variables.events[event].listeners.filter((listener) => {
